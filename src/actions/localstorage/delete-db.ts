...
export const deleteFilesDbByName = async (fileName: string) => {
    const e = await getTranslations('Error');
    const f = await getTranslations('Files')
    const s = await getTranslations('System')

    try {
        const fileexists = await prisma.files.findFirst({ where: { name: fileName } })
        if (!fileexists) {
            return { status: 404, data: { message: f("filedoesnotexist") } }
        }

        await prisma.files.deleteMany({ where: { name: fileName } })
